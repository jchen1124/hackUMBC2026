...
def process_all_conversations(db_path: str, contact_handle_ids: list[int]) -> str:
    """
    Process conversations and return a summary string.
    Always returns a string, never None.
    """
    if not os.path.exists(db_path):
        return "Error: Messages database not found. Make sure you're running this on macOS with Messages app enabled."
    ...
    try:
        ...
        for handle_id, messages_for_contact in itertools.groupby(cursor, key=itemgetter('handle_id')):
            ...
            # Convert Apple timestamps & group by year-month
            def ym_key(row):
            ...
            for month_key, messages_for_month in itertools.groupby(messages_for_contact, key=ym_key):
                ...
                # Only process if we have enough messages to summarize
                if message_count < 3:
                    continue
